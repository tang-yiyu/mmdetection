...
class TwoChannelImgDataPreprocessor(ImgDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> Union[dict, list]:
        ...
        # rgb
        _batch_inputs_rgb = data['inputs_rgb']
        # Process data with `pseudo_collate`.
        if is_seq_of(_batch_inputs_rgb, torch.Tensor):
            batch_inputs_rgb = []
            for _batch_input_rgb in _batch_inputs_rgb:
                # channel transform
                if self._channel_conversion:
                    _batch_input_rgb = _batch_input_rgb[[2, 1, 0], ...]
                # Convert to float after channel conversion to ensure
                # efficiency
                _batch_input_rgb = _batch_input_rgb.float()
                # Normalization.
                if self._enable_normalize:
                    if self.mean.shape[0] == 3:
                        assert _batch_input_rgb.dim(
                        ) == 3 and _batch_input_rgb.shape[0] == 3, (
                            'If the mean has 3 values, the input tensor '
                            'should in shape of (3, H, W), but got the tensor '
                            f'with shape {_batch_input_rgb.shape}')
                    _batch_input_rgb = (_batch_input_rgb - self.mean) / self.std
                batch_inputs_rgb.append(_batch_input_rgb)
            # Pad and stack Tensor.
            batch_inputs_rgb = stack_batch(batch_inputs_rgb, self.pad_size_divisor,
                                       self.pad_value)
        # Process data with `default_collate`.
        elif isinstance(_batch_inputs_rgb, torch.Tensor):
            assert _batch_inputs_rgb.dim() == 4, (
                'The input of `ImgDataPreprocessor` should be a NCHW tensor '
                'or a list of tensor, but got a tensor with shape: '
                f'{_batch_inputs_rgb.shape}')
            if self._channel_conversion:
                _batch_inputs_rgb = _batch_inputs_rgb[:, [2, 1, 0], ...]
            # Convert to float after channel conversion to ensure
            # efficiency
            _batch_inputs_rgb = _batch_inputs_rgb.float()
            if self._enable_normalize:
                _batch_inputs_rgb = (_batch_inputs_rgb - self.mean) / self.std
            h, w = _batch_inputs_rgb.shape[2:]
            target_h = math.ceil(
            ...
            pad_h = target_h - h
            pad_w = target_w - w
            batch_inputs_rgb = F.pad(_batch_inputs_rgb, (0, pad_w, 0, pad_h),
                                 'constant', self.pad_value)
        else:
            raise TypeError('Output of `cast_data` should be a dict of '
                            'list/tuple with inputs and data_samples, '
                            f'but got {type(data)}: {data}')
        data['inputs_rgb'] = batch_inputs_rgb
        ...
        if is_seq_of(_batch_inputs_ir, torch.Tensor):
            ...
            for _batch_input_ir in _batch_inputs_ir:
                ...
                if self._enable_normalize:
                    ...
                    _batch_input_ir = (_batch_input_ir - self.mean) / self.std
        ...
        # Process data with `default_collate`.
            ...
            if self._enable_normalize:
                _batch_inputs_ir = (_batch_inputs_ir - self.mean) / self.std
...
class TwoChannelDetDataPreprocessor(TwoChannelImgDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        if data_samples is not None:
            ...
            if inputs_rgb[0].size()[-2:] == inputs_ir[0].size()[-2:]:
                batch_input_shape = tuple(inputs_rgb[0].size()[-2:])
            else:
                raise ValueError('The input image size of RGB and IR should be the same.')
        ...
        # inputs_ir_1 = inputs[:, 3:, :, :]

        # if torch.equal(inputs_rgb, inputs_rgb_1) and torch.equal(inputs_ir, inputs_ir_1):
        #     print('equal')
        # else:
        #     print('not equal')
